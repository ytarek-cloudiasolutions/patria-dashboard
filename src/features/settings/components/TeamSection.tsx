...
const TeamSection = ({
  ...
  return (
    <SectionCard
      ...
      subtitle={t("Manage administrator roles and access levels")}
      contentClassName="px-0 py-0 sm:px-0 sm:py-0"
      action={
        <DefaultButton
          data={{
            buttonText: t("Invite Member"),
        ...
        <Table className="border-0">
          <TableHeader className="bg-white [&_tr:hover]:bg-white">
            <TableRow className="relative after:absolute after:inset-x-2 after:bottom-0 after:h-px after:bg-[#E5E5E5]">
              <TableHead className="ps-6 py-4 text-start">{t("MEMBER")}</TableHead>
              <TableHead className="px-6 py-4 text-center">{t("CONTACT")}</TableHead>
              <TableHead className="px-6 py-4 text-center">{t("ROLE")}</TableHead>
              <TableHead className="px-6 py-4 text-center">{t("PERFORMANCE")}</TableHead>
              <TableHead className="pe-6 py-4 text-end">{t("PERMISSIONS")}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {members.map((member) => (
              <TableRow key={member.id} className="hover:bg-[#FAFAF8]">
                ...
                <TableCell className="pe-6 py-4 text-end">
