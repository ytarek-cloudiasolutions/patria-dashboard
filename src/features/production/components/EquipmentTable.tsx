...
import type { EquipmentRecord, MaintenanceStatus, MaintenanceTaskType } from "../types";
...
const STATUS_STYLES: Record<MaintenanceStatus, string> = {
  Completed: "bg-[#E2F4ED] text-[#059B5A] border-current",
  Pending: "bg-[#FFF5DC] text-[#B56C00] border-current",
  Overdue: "bg-[#FFF0F0] text-[#C90000] border-current",
};

const TaskBadge = ({ task }: { task: MaintenanceTaskType }) => {
  const { t } = useTranslation();
  return (
    <Badge className="h-6 rounded-full border px-3 py-0 text-[11px] font-semibold border-[#595959] bg-[#E5E5E5] text-[#23252A]">
      {t(task)}
    </Badge>
  );
};
    ...
    <>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 text-[13px]">
              <div>
                ...
                <TaskBadge task={record.taskType} />
              </div>
              <div>
                <p className="mb-0.5 text-[11px] font-semibold uppercase tracking-wide text-[#8B8B8B]">
                  {t("Operator")}
      ...
      <div className="hidden md:block">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="ps-6 py-4 text-start">{t("EQUIPMENT")}</TableHead>
